<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :default-sort="{prop: 'activity_endtime', order: 'descending'}"
    ...
    >
      ...
      <el-table-column label="活动ID" width="200" align="center" :sortable="true" prop="activity_id">
        <template slot-scope="scope">{{ scope.row.activity_id }}</template>
      </el-table-column>
      <el-table-column label="活动名称" align="center" width="240" :sortable="true" prop="activity_name">
      ...
      >
        <template slot-scope="scope">
          ...
          <span>{{ scope.row.activity_begintime }}</span>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
      ...
      >
        <template slot-scope="scope">
          <i class="el-icon-time" />
          <span>{{ scope.row.activity_endtime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="活动成绩" align="center">
          ...
          >
            <div class="demo-drawer__content">
              <el-form :model="form">
                ...
                <el-form-item label="活动成绩" :label-width="formLabelWidth" style="text-align:left">
...
<script>
import { ActivityUpdate, ActivityDelete, getActivityList } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      ActivityDelete(row).then(response => {
        getActivityList().then(response => {
          this.list = response.data.items
          this.total = response.data.total
        })
      ...
      this.$confirm('确定要提交表单吗？')
        .then(_ => {
          ...
          this.timer = setTimeout(() => {
            ActivityUpdate(this.form).then(response => {
              getActivityList().then(response => {
                this.list = response.data.items
                this.total = response.data.total
              })
