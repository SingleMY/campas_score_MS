<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :default-sort="{prop: 'p_prodution_date', order: 'descending'}"
    ...
    >
      ...
      <el-table-column label="产品ID" width="180" align="center" :sortable="true" prop="p_id">
        <template slot-scope="scope">{{ scope.row.p_id }}</template>
      </el-table-column>
      <el-table-column label="产品名称" align="center" :sortable="true" prop="p_name">
        <template slot-scope="scope">
          <span>{{ scope.row.p_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="产品价格" align="center" width="140" :sortable="true" prop="p_price">
        <template slot-scope="scope">
      ...
      <el-table-column
        ...
        width="200"
        :sortable="true"
        prop="p_prodution_date"
      >
        <template slot-scope="scope">
          <i class="el-icon-time" />
          <span>{{ scope.row.p_prodution_date }}</span>
        </template>
      </el-table-column>
      <el-table-column label="保质期" align="center" width="140" :sortable="true" prop="p_validity">
        <template slot-scope="scope">
          <span>{{ scope.row.p_validity }}</span>
        </template>
      </el-table-column>
          ...
          >
            <div class="demo-drawer__content">
              <el-form :model="form">
                ...
                <el-form-item label="生产地" :label-width="formLabelWidth">
                  <el-input
                ...
                <el-form-item label="保质期" :label-width="formLabelWidth" style="text-align:left">
                  <el-input v-model="form.p_validity" />
                </el-form-item>
              </el-form>
...
<script>
import { ProductionUpdate, ProductionDelete, getProductionList } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      ProductionDelete(row).then(response => {
        getProductionList().then(response => {
          this.list = response.data.items
          this.total = response.data.total
        })
      ...
      this.$confirm('确定要提交表单吗？')
        .then(_ => {
          ...
          this.timer = setTimeout(() => {
            ProductionUpdate(this.form).then(response => {
              getProductionList().then(response => {
                this.list = response.data.items
                this.total = response.data.total
              })
